...
const canvas = d3.select(".canva-graph7");
...
d3.json("Cross-sections-json.txt").then(function(graph1data) { // Corrected filename here
    ...
    node.append("text")
        ...
        .style("font-size", "8px"); // Increased font size slightly for better readability

}).catch(function(error) {
    console.error("Error loading or processing JSON:", error); // Corrected log message
});
}
createGraph1(".canva-graph7", "Cross-sections-json.txt");
...
const canvas = d3.select(".canva-graph8")
...
//Stratify the data for 1st graph
d3.json("Cross-sections-IS-SC-json.txt").then(function(graph2data) { // Corrected filename here
    // Log the raw flat data to see how D3 parsed it
    ...
    // Compute the tree height.
    const dx = 15;
    const dy = width / (root.height + 1.9); // Use 'root' instead of 'root1' // adjust link length here
    ...
    // Introduce a left margin for the viewBox
    const leftMargin = 100; // Adjust this value as needed to move the graph right
    ...
    node.append("text")
        ...
        .attr("stroke", "white")
        .attr("paint-order", "stroke")
        .style("font-size", "7px"); // Increased font size slightly for better readability

}).catch(function(error) {
    console.error("Error loading or processing JSON:", error); // Corrected log message
})
}
createGraph2(".canva-graph8", "Cross-sections-IS-SC.txt");